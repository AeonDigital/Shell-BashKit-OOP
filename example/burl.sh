...
burlPrintHeaders() {
  ...
  for key in $(varAssocKeys "asc" "tmpAssoc"); do
    value="${tmpAssoc[${key}]}"
...
burlSetVerb() {
  ...
  BASHKIT_MAIN_OBJECT_INSTANCES_VALUES["${regTypeInstanceMemberName}"]="${strTmpSet}"
}
...
burlClear() {
  ...
  for typeInstanceMemberName in "${!assocProp[@]}"; do
    ...
    case "${typeInstanceMemberName}" in
      "header")
        varAssocClear "${BASHKIT_MAIN_OBJECT_INSTANCES_VALUES["${regTypeInstanceMemberName}"]}"
        ;;
      "verb")
        BASHKIT_MAIN_OBJECT_INSTANCES_VALUES["${regTypeInstanceMemberName}"]="GET"
        ;;
      "protocol")
        BASHKIT_MAIN_OBJECT_INSTANCES_VALUES["${regTypeInstanceMemberName}"]="http"
        ;;
      "protocolVersion")
        BASHKIT_MAIN_OBJECT_INSTANCES_VALUES["${regTypeInstanceMemberName}"]="1.1"
        ;;
      "domain")
        BASHKIT_MAIN_OBJECT_INSTANCES_VALUES["${regTypeInstanceMemberName}"]=""
        ;;
      "port")
        BASHKIT_MAIN_OBJECT_INSTANCES_VALUES["${regTypeInstanceMemberName}"]="80"
        ;;
      "path")
        BASHKIT_MAIN_OBJECT_INSTANCES_VALUES["${regTypeInstanceMemberName}"]=""
        ;;
